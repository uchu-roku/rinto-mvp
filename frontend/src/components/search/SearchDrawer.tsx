...
export default function SearchDrawer({
  open, onClose, speciesOptions, features = [], onApply,
}: Props) {
  ...
  return (
    <div style={{ ...drawer, transform: open ? "translateX(0)" : "translateX(-110%)" }}>
      ...
      <div style={grid}>
        <label>樹高 (m) 最小
          <input value={minHeight} onChange={e => setMinHeight(e.target.value)} placeholder="例: 5" />
        </label>
        <label>樹高 (m) 最大
          <input value={maxHeight} onChange={e => setMaxHeight(e.target.value)} placeholder="例: 25" />
        </label>
        <label>直径 DBH (cm) 最小
          <input value={minDbh} onChange={e => setMinDbh(e.target.value)} placeholder="例: 8" />
        </label>
        <label>直径 DBH (cm) 最大
          <input value={maxDbh} onChange={e => setMaxDbh(e.target.value)} placeholder="例: 30" />
        </label>

        <div>
          <div style={{ marginBottom: 6 }}>樹種（複数選択）</div>
          <div style={{ display: "grid", gap: 6, maxHeight: 140, overflow: "auto", border: "1px solid #e5e7eb", borderRadius: 8, padding: 8 }}>
              ...
              return (
                <label key={s} style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  <input
                    ...
                    checked={checked}
                    onChange={() =>
                      setSpecies(checked ? species.filter(x=>x!==s) : [...species, s])
                    }
                  />
                  <span>{s}</span>
                </label>
      ...
      <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
        <button onClick={apply} style={btnPrimary}>この条件で適用</button>
        <button onClick={exportCSV} style={btn}>CSV出力</button>
      </div>

      <div style={statsBox}>
        <div style={{ fontWeight: 700, marginBottom: 6 }}>統計</div>
        <div>件数: <b>{stats.count}</b></div>
        <div>平均樹高 (m): <b>{fmt(stats.avg_height_m, 2)}</b></div>
        <div>平均直径 DBH (cm): <b>{fmt(stats.avg_dbh_cm, 1)}</b></div>
        <div>総材積 (m³): <b>{fmt(stats.total_volume_m3, 3)}</b></div>
      </div>
    </div>
  );
}
...
const head: React.CSSProperties = { display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 };
const grid: React.CSSProperties = { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 };
const btn: React.CSSProperties = { border: "1px solid #e5e7eb", background: "#f9fafb", padding: "6px 10px", borderRadius: 8, cursor: "pointer" };
const btnPrimary: React.CSSProperties = { ...btn, background: "#111827", color: "#fff", borderColor: "#111827" };
const statsBox: React.CSSProperties = { marginTop: 12, borderTop: "1px solid #e5e7eb", paddingTop: 8, fontSize: 13 };
